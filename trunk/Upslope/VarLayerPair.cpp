...
cmf::upslope::VariableLayerSaturated* cmf::upslope::VariableLayerSaturated::Create(Cell& cell,real lowerboundary,const RetentionCurve& r_curve)
{
	...
	upperLayer->Name="Unsaturated zone (" + cell.ToString() + ")";
	lowerlayer->Name="Saturated zone (" + cell.ToString() + ")";
...
cmf::upslope::VariableLayerUnsaturated::VariableLayerUnsaturated(cmf::upslope::Cell & cell,real upperboundary,real lowerboundary,const RetentionCurve & r_curve,cmf::upslope::VariableLayerSaturated* LayerBelow ) 
...
{
	return cmf::upslope::SoilWaterStorage::get_potential();
...
{
	return cmf::upslope::SoilWaterStorage::get_wetness();
	real f_empty=/*square*/(piecewise_linear(-get_thickness(),-0.1,-0.01));
	return f_empty*belowLayer->get_wetness() + (1-f_empty)*get_state()/get_capacity();
}

real cmf::upslope::VariableLayerUnsaturated::get_lower_boundary() const
{
	real sat_ub=belowLayer ? belowLayer->get_upper_boundary() : get_upper_boundary()+0.001;
