...
outlet=p.NewOutlet('outlet',outcell.x-10, outcell.y, outcell.z-.3)
# Connect outlet to soil
#outcell.connect_soil_with_node(outlet,cmf.Richards_lateral,10.0,5.0)
# Connect outlet to surfacewater (overbank flow)
cmf.Manning_Kinematic(outcell.surfacewater, outlet, 
...
if __name__ == '__main__':
    def as_raster(f):
        ...
        for c in p:
            res[c.x,c.y] = f(c)
        return res
    f=lambda c: c.saturated_depth
    def run(dt,vmax=2,qscale=200):
        solver(dt)
        as_raster(f).draw(hold=0,vmin=0,vmax=vmax)
        cmf.draw.cell_quiver(p,solver.t,color='w',scale=qscale)
        title('%s, dt=%s, P=%0.1f mm/day' % (solver.t,solver.dt, p.rainfall_stations[0].data[solver.t]))
    if not 'setup' in sys.argv:
