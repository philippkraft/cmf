...
REM mkdir %SND_SRC%
REM git clone %SND_URL% %SND_SRC%
REM RD /Q /F /S %SND_SRC%\.git
...
cmake .. -G "NMake Makefiles" -DBUILD_ARKODE=OFF -DBUILD_CVODES=OFF -DBUILD_IDA=OFF -DBUILD_IDAS=OFF -DBUILD_KINSOL=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX=%SND_LIB_DIR% -DEXAMPLES_INSTALL=OFF -DKLU_ENABLE=ON -DKLU_LIBRARY_DIR=%KLU_LIB_DIR%/lib -DKLU_INCLUDE_DIR=%KLU_LIB_DIR%/include/suitesparse -DOPENMP_ENABLE=ON -DCMAKE_BUILD_TYPE=Release
REM cmake .. -DKLU_ENABLE=ON -DKLU_LIBRARY_DIR=%KLU_LIB_DIR%/lib -DKLU_INCLUDE_DIR=%KLU_LIB_DIR%/include
