...
plt.imshow(plt.transpose(perc_mx), cmap=plt.plt.cm.Blues, aspect='auto', interpolation='nearest', vmin=0.0,
           vmax=rainfall)
...
plt.imshow(plt.transpose(perc_mp), cmap=plt.plt.cm.Blues, aspect='auto', interpolation='nearest', vmin=0.0,
           vmax=rainfall)
plt.xticks(plt.arange(0, 10 * 24 * 12, 24 * 12), plt.arange(0, 10))
plt.xlabel('days')
plt.colorbar()
...
plt.suptitle('$K_{sat}^{mx}=%g m/day, K_{sat}^{mp}=%g m/day, \Phi_{mp}=%g$' % (
c.layers[0].Ksat, macropores[0].Ksat, porefraction), size=20)
