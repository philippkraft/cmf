...
// 			}
		...
		class Raster
		{
			...
			public:
				...
				header(int _ncols,int _nrows, double _xllcorner, double _yllcorner, double _xcellsize,double _ycellsize,rasterType nodata=-9999)
				{
...
#pragma omp parallel for
		...
		private:
			...
			{
				return IdentifyColRow(window.col(index),window.row(index));
			}
			...
			{
				return abs(window_mean(window)-IdentifyColRow(window.center_col,window.center_row));
			}
